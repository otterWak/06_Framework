...
- Promise : 비동기 작업의 결과를 처리하는 방법 중 하나
...
function getTotalCount(){ // 함수 정의
  ...
  .then( response => { // fetch가 서버에서 응답을 받으면,
    ...
    // response.text() : 응답 데이터를 문자열/숫자 형태로 반환한 결과를 가지는
    //                   Promise 객체 반환
  ...
  .then(result => { // 첫 번째 콜백함수가 완료된 후 호출
    ...
    // #totalCount 요소의 내용으로 result를 대입
...
addBtn.addEventListener("click", ()=> {
  ...
  const param = {
    // key : value
    "todoTitle" : todoTitle.value.trim(),
    "todoContent" : todoContent.value.trim()
  }
...
deleteBtn.addEventListener("click", ()=>{
  ...
  fetch("/ajax/delete", {
    ...
    // -> 원래는 이렇게 명시하는게 옳음(엄격한 환경에서는 이렇게 작성해야 함)
  })
  .then( resp => resp.text() )
  .then( result => {

...
public class EditBoardController {
	...
	public String boardUpdate(@PathVariable("boardCode") int boardCode,
		...
		if(board == null) {
			...
			path = "board/boardUpdate"; // templates/board/boardUpdate.html
	 ...
	 * @param loginMember	   : 로그인한 회원의 번호 이용 (로그인 == 작성자)
	 ...
	 * @param cp(current page) : 수정 성공 시 이전 파라미터 유지 
	 * @return
	 */
	...
	public String boardUpdate(@PathVariable("boardCode") int boardCode,
		...
		if(result > 0) {
			...
			message = "수정 실패";
			path = "update"; // GET (수정 화면 전환) 리다이렉트하는 상대 경로
		}

...
@Transactional(rollbackFor=Exception.class) // rollbackFor 속성은 적지 않아도 됨
@Service
@Slf4j
public class MemberServiceImpl implements MemberService{
	...
	public Member login(Member inputMember) {
		...
		// bycrypt.encode(문자열) : 문자열을 암호화하여 반환
		...
		//	  암호와된 비밀번호(loginMember.getMemberPw())
		// 	  두 비밀번호가 일치하는지 확인
	...
	public int signup(Member inputMember, String[] memberAddress) {
		...
		// 주소가 입력되지 않으면
		// inputMember.getMemberAddress(0 -> ",,")
		// memberAddress -> [,,]
		...
		if( !inputMember.getMemberAddress().equals(",,")) {
			...
			// [13289, 경기 과천시 대공원광장로 102, 대공원 호랑이 우리]
			// 13829^^^경기 과천시 대공원광장로 102^^^대공원 호랑이 우리

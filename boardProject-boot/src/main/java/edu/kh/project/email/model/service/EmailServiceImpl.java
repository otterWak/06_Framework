...
public class EmailServiceImpl implements EmailService{
	...
	// SpringTemplateEngine : 타임리프(템플릿 엔진)를 이용해서
	//						  html 코드 -> java 코드 변환
	...
	public String sendEmail(String htmlName, String email) {
		...
		try {
			...
			// HTML 내용 설정
			// true : 해석해서 보낼 것인지(기본 값 : false)
			// 인증번호입니다 : ag2dcd
			// hlper.setText("인증번호입니다 : " + authKey);
			helper.setText( loadHtml(authKey,htmlName), true ); 
			
			// 메일에 이미지 첨부(로고)
			// resouces 타입을 바라기 때무에 ClassPathResource 사용
	...
	private String loadHtml(String authKey, String htmlName) {
		...
		// Context : Thymeleaf에서 제공하는 HTML 템플릿에
	...
	// 메서드 레벨에서도 이용 가능(해당 메서드에서만 트랜잭션 커밋/롤백)
